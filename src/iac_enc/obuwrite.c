...
uint32_t iac_obu_memmove(uint32_t obu_header_size, uint32_t obu_payload_size,
  uint8_t *data) {
...
int iac_write_uleb_obu_size(uint32_t obu_header_size, uint32_t obu_payload_size,
  uint8_t *dest) {
...
uint32_t iac_write_obu_unit(const uint8_t * sampledata, int playload_size,
  ...
  obu_header_size = iac_write_obu_header(obu_type, 0, dst);
  const uint32_t length_field_size = uleb_size_in_bytes(playload_size);
  iac_write_uleb_obu_size(obu_header_size, playload_size, dst);
