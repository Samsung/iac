...
static int dmx_s7(Demixer *ths)
{
    ...
    if (!ths->ch_data[IA_CH_BR7]) {
        ...
        for (; i < ths->skip; i++) {
            l[i] = (ths->ch_data[IA_CH_SL5][i] - ths->ch_data[IA_CH_SL7][i] *
                    demixing_type_mat[last_Typeid].alpha) / demixing_type_mat[last_Typeid].beta;
            r[i] = (ths->ch_data[IA_CH_SR5][i] - ths->ch_data[IA_CH_SR7][i] *
                    demixing_type_mat[last_Typeid].alpha) / demixing_type_mat[last_Typeid].beta;
        }

        for (; i < ths->frame_size; i++) {
            l[i] = (ths->ch_data[IA_CH_SL5][i] - ths->ch_data[IA_CH_SL7][i] *
                    demixing_type_mat[Typeid].alpha) / demixing_type_mat[Typeid].beta;
            r[i] = (ths->ch_data[IA_CH_SR5][i] - ths->ch_data[IA_CH_SR7][i] *
                    demixing_type_mat[Typeid].alpha) / demixing_type_mat[Typeid].beta;
        }
...
static int dmx_h2 (Demixer *ths)
{
    ...
    if (!ths->ch_data[IA_CH_HR]) {
        ...
        w = calc_w (WeightTypeNum, ths->last_weight_state_idx, &ths->weight_state_idx);
        ...
        for (; i < ths->skip; i++) {
            l[i] = ths->ch_data[IA_CH_TL][i] -
                   demixing_type_mat[last_Typeid].delta * lastW * ths->ch_data[IA_CH_SL5][i];
            r[i] = ths->ch_data[IA_CH_TR][i] -
                   demixing_type_mat[last_Typeid].delta * lastW * ths->ch_data[IA_CH_SR5][i];
        }
...
Demixer *demixer_open(uint32_t frame_size, uint32_t delay)
{
    ...
    if (ths) {
        ...
        for (int i = 0; i < windowLen; i++) {
            ths->hanning_filter[i] = (0.5 * (1.0 - cos(2.0*M_PI*(double)i / (double)(
                                                 windowLen - 1))));
        }
...
int demixer_set_demixing_mode (Demixer *ths, int mode)
{
    ...
    if ((mode >= 0 && mode < 3) || (mode > 3 && mode < 7)) {
        ...
        ia_logd("last weight state index : %d -> %d", ths->last_weight_state_idx, ths->weight_state_idx);
