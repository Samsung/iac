...
void TfLiteTensorDataFree(TfLiteTensor* t) {
  //TODO , refer to SetTensorParametersReadWrite() and SetTensorParametersReadOnly()
  if (t->allocation_type == kTfLiteDynamic || t->allocation_type == kTfLiteArenaRw || t->allocation_type == kTfLiteArenaRwPersistent) {
    free(t->data.raw);
  }
  t->data.raw = NULL;
}
...
void *GetTensorData(TfLiteTensor* tensor)
{
  if(tensor == NULL){
      return NULL;
  }
  return  (void*)(tensor->data.raw);
}


TfLiteIntArray* GetTensorShape(TfLiteTensor* tensor)
{
  if(tensor == NULL){
      return NULL;
  }
...
int MatchingDim(TfLiteIntArray* input, int dim1,TfLiteIntArray* output , int dim2)
{
  return  (input->data[dim1] < output->data[dim2] ) ? input->data[dim1]:output->data[dim2];
}

int Offset(const TfLiteIntArray* shape, int i0, int i1, int i2, int i3) {
