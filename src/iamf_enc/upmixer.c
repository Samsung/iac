...
static int upmix_s2(UpMixer *um, int *w_x)
{
...
static int upmix_hf2toh2(UpMixer *um, int *w_x)
{
  Mdhr current = um->mdhr_c, last = um->mdhr_l;
  int last_Typeid = last.dmix_matrix_type - 1;
  float last_w_z = calc_w(last.weight_type, um->last_weight_state_value_x_prev2_int, w_x);
  int DmixTypeNum = current.dmix_matrix_type;
  int WeightTypeNum = current.weight_type;
  int Typeid = DmixTypeNum - 1;
  float w_z = calc_w(WeightTypeNum, um->last_weight_state_value_x_prev_int, w_x);
...
static int upmix_h2(UpMixer *um, int *w_x)
{
...
void upmix_to100(UpMixer *um, int *w_x)
{
...
void upmix(UpMixer *um, const unsigned char *gain_down_map)
{
  ...
  for (int i = 0; i < CHANNEL_LAYOUT_MAX; i++)
  {
    ...
    if (layout == CHANNEL_LAYOUT_MAX)
      break;
    conv_upmixpcm(um->up_input[layout], dspInBuf, enc_get_layout_channel_count(layout) - pre_ch, start_ch, um->frame_size);
  ...
  for (int i = 0; i < CHANNEL_LAYOUT_MAX; i++)
  {
    ...
    {
      ((void(*)(UpMixer *um, int *w_x))g_upmix[layout].data)(um, &weight_state_value_x_curr_int);
    }
  }
  ...
  um->last_weight_state_value_x_prev2_int = um->last_weight_state_value_x_prev_int;
