...
static int mov_write_aiac_tag(MOVMuxContext *movm)
{
  ...
  size += avio_w8(audio_t->aiac.sub_bitstream_count);  
  return size;
}
...
static int mov_write_sgpd_tag(MOVMuxContext *movm) // non-timed metadata
{
  ...
  OpusHeader *header = (OpusHeader *)audio_t->aiac.csc;
	...
	// grouping type
	size += avio_wstring("iant");// IA non-timed metadata
  ...
  // Codec_Specific_Info--
  bs_setbits(&bs, audio_t->aiac.codec_id, 32);
  bs_setbits(&bs, 0, 32);
  // version
  bs_setbits(&bs, header->version, 8);
  // OutputChannelCount
  bs_setbits(&bs, header->output_channel_count, 8);
  // PreSkip
  bs_setbits(&bs, header->preskip, 16);
  // InputSampleRate
  bs_setbits(&bs, header->input_sample_rate, 32);
  // OutputGain
  bs_setbits(&bs, header->output_gain, 16);
  // ChannelMappingFamily
  bs_setbits(&bs, header->channel_mapping_family, 8);

  // ChannelAudioLayerConfiguration--
  ...
  bs_setbits(&bs, 0, 2); // reserved
  bs_setbits(&bs, audio_t->ia_static_meta.substream_size_is_present_flag, 1); //
  bs_setpadbits(&bs);
  //size += avio_wdata(bitstr, bs.m_posBase); // total is 1 byte

  if (audio_t->ia_static_meta.ambisonics_mode == 2)
  {
    bs_setbits(&bs, audio_t->ia_static_meta.amb_layer_config.output_channel_count, 8); //
    bs_setbits(&bs, audio_t->ia_static_meta.amb_layer_config.substream_count, 8); //
    bs_setbits(&bs, audio_t->ia_static_meta.amb_layer_config.coupled_substream_count, 8); //
    for (int i = 0; i < audio_t->ia_static_meta.amb_layer_config.output_channel_count; i++)
    {
      bs_setbits(&bs, audio_t->ia_static_meta.amb_layer_config.channel_mapping[i], 8); //
    }
  }
  ...
  if (audio_t->ia_static_meta.ambisonics_mode == 3)
  {
    ...
    bs_setbits(&bs, audio_t->ia_static_meta.amb_layer_config.substream_count, 8); //
    bs_setbits(&bs, audio_t->ia_static_meta.amb_layer_config.coupled_substream_count, 8); //
    int channel_count = audio_t->ia_static_meta.amb_layer_config.output_channel_count;
    int stream_count = audio_t->ia_static_meta.amb_layer_config.substream_count;
    int coupled_stream_count = audio_t->ia_static_meta.amb_layer_config.coupled_substream_count;
    int dmatrix_size = (stream_count + coupled_stream_count) * channel_count;

    for (int i = 0; i < dmatrix_size; i++)
    {
      bs_setbits(&bs, audio_t->ia_static_meta.amb_layer_config.demixing_matrix[i], 16); //
    }
  }
  for (int i = 0; i < audio_t->ia_static_meta.channel_audio_layer; i++)
  {

    bs_setbits(&bs, audio_t->ia_static_meta.cha_layer_config[i].loudspeaker_layout, 4);
    bs_setbits(&bs, audio_t->ia_static_meta.cha_layer_config[i].output_gain_is_present_flag, 1);
    bs_setbits(&bs, audio_t->ia_static_meta.cha_layer_config[i].recon_gain_is_present_flag, 1);
    bs_setbits(&bs, 0, 2);
    bs_setbits(&bs, audio_t->ia_static_meta.cha_layer_config[i].substream_count, 8);
    bs_setbits(&bs, audio_t->ia_static_meta.cha_layer_config[i].coupled_substream_count, 8);
    bs_setbits(&bs, audio_t->ia_static_meta.cha_layer_config[i].loudness, 16);
    if(audio_t->ia_static_meta.cha_layer_config[i].output_gain_is_present_flag)
      bs_setbits(&bs, audio_t->ia_static_meta.cha_layer_config[i].output_gain, 16);

    bs_setpadbits(&bs);

  }
  // Default length
  size += avio_wb32(bs.m_posBase);
  // entry count
  size += avio_wb32(1);
