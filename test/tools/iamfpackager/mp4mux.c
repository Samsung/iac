...
static int mov_write_dops_tag(MOVMuxContext *movm)
{
  ...
  OpusHeader *header = (OpusHeader *)audio_t->iamf.csc;
  // version
...
static int mov_write_esds_tag(MOVMuxContext *movm)
{
  ...
  */
  ...
  mov_audio_track *audio_t = &movm->audio_trak[audio_trak_select];
  int size = 0;
  AacHeader *header = (AacHeader *)audio_t->iamf.csc;
...
static int mov_write_sgpd_tag(MOVMuxContext *movm) // Global Descriptor Sample Group
{
  int audio_trak_select;
  audio_trak_select = movm->audio_trak_select;
  mov_audio_track *audio_t = NULL;
  ...
  OpusHeader *header = (OpusHeader *)audio_t->iamf.csc;
	...
	// grouping type
	size += avio_wstring("iagd");// Global Descriptor Sample Group

  int size_of_mix_presentations_group_entry = audio_t->ia_descriptor.size_of_mix_presentations_group_entry;
  int size_of_audio_elements_group_entry = audio_t->ia_descriptor.size_of_audio_elements_group_entry;
  // Default length
  size += avio_wb32(size_of_mix_presentations_group_entry + size_of_audio_elements_group_entry);
  // entry count
  size += avio_wb32(1);
  size += avio_wdata(audio_t->ia_descriptor.mix_presentations_group_entry, size_of_mix_presentations_group_entry);
  size += avio_wdata(audio_t->ia_descriptor.audio_elements_group_entry, size_of_audio_elements_group_entry);
	return size;
}

static int mov_write_sbgp_tag(MOVMuxContext *movm) // Global Descriptor Sample Group
{
	int size = 0;
...
static int mov_write_sgpd2_tag(MOVMuxContext *movm) // Demixing Info Sample Group
{
  int audio_trak_select;
  audio_trak_select = movm->audio_trak_select;
  mov_audio_track *audio_t = NULL;
  ...
  size += avio_wb32(0x01000000);
  // grouping type
  size += avio_wstring("demi");// demixing parameter
  // Default length                          
  size += avio_wb32(audio_t->demixing_info.size_of_demix_group_entry); // OBU size,demxing_info_obu_size
  // entry count
  size += avio_wb32(audio_t->demixing_info.demix_entry_count1);
  for (int i = 0; i < audio_t->demixing_info.demix_entry_count1; i++)
  {
    avio_wdata(audio_t->demixing_info.demix_group_entry[i], audio_t->demixing_info.size_of_demix_group_entry);
    size += audio_t->demixing_info.size_of_demix_group_entry;
  }
  return size;
}

static int mov_write_sbgp2_tag(MOVMuxContext *movm) // Demixing Info Sample Group
{
  // Table data
  int audio_trak_select;
  mov_audio_track *audio_t = NULL;
  ...
  size += avio_wb32(0x00000000);
  // grouping type
  size += avio_wstring("demi");// demixing prameter
  // entry count                          
  size += avio_wb32(audio_t->demixing_info.demix_entry_count2);
  for (int i = 0; i < audio_t->demixing_info.demix_entry_count2; i++)
  {
    size += avio_wb32(audio_t->demixing_info.demix_sample_count[i]);
    size += avio_wb32(audio_t->demixing_info.demix_group_description_index[i]);
  }
...
*/
static int mov_write_sgpd3_tag(MOVMuxContext *movm) // Global Descriptor Sample Group
{
  int audio_trak_select;
  audio_trak_select = movm->audio_trak_select;
  mov_audio_track *audio_t = NULL;
...
static int mov_write_sbgp3_tag(MOVMuxContext *movm) // Global Descriptor Sample Group
{
  int size = 0;
...
static avio_context atoms_opustrak[] = {
  ...
  { MOV_ATOM_DATA, mov_write_sbgp2_tag },
  { MOV_ATOM_NAME, "sgpd" },
  { MOV_ATOM_DATA, mov_write_sgpd3_tag },
  { MOV_ATOM_NAME, "sbgp" },
  { MOV_ATOM_DATA, mov_write_sbgp3_tag },
  { MOV_ATOM_UP },
  { MOV_ATOM_UP },
  { MOV_ATOM_UP },
  { MOV_ATOM_UP },
...
static int mp4_flush_segment(MOVMuxContext *movm)
{
  ...
  for (int i = 0; i < audio_t->demixing_info.demix_entry_count2; i++)
  {
    audio_t->demixing_info.demix_sample_count[i] = 0;
    audio_t->demixing_info.demix_group_description_index[i] = 0;
  }
  audio_t->demixing_info.demix_entry_count1 = 0;
  audio_t->demixing_info.demix_entry_count2 = 0;
  audio_t->demixing_info.demix_group_entry_pointer = 0;
...
int mp4_demix_info2(mov_audio_track *audio_t, int size_of_demix_group, uint8_t * demix_group)
{
  if (size_of_demix_group <= 0)
  {
    return -1;
  }
  int demix_entry_count2 = audio_t->demixing_info.demix_entry_count2;
  int demix_group_entry_pointer = audio_t->demixing_info.demix_group_entry_pointer;
  if (demix_entry_count2 == 0)
  {
    audio_t->demixing_info.demix_sample_count[demix_entry_count2] = 1;
    audio_t->demixing_info.demix_group_description_index[demix_entry_count2] = 1;
    audio_t->demixing_info.demix_entry_count2++;
  }
  else if (memcmp (audio_t->demixing_info.demix_group_entry[demix_group_entry_pointer], demix_group, size_of_demix_group) == 0)
  {
    audio_t->demixing_info.demix_sample_count[demix_entry_count2 - 1]++;
  }
  else
  {
    audio_t->demixing_info.demix_entry_count2++;
    demix_entry_count2 = audio_t->demixing_info.demix_entry_count2;
    audio_t->demixing_info.demix_sample_count[demix_entry_count2 - 1] = 1;
    int find_i = 0;
    for (find_i = 0; find_i < audio_t->demixing_info.demix_entry_count1; find_i++)
    {
      if (memcmp(demix_group, audio_t->demixing_info.demix_group_entry[find_i], size_of_demix_group) == 0)
        break;
    }
    audio_t->demixing_info.demix_group_description_index[demix_entry_count2 - 1] = find_i + 1;
  }

  int index = 0;
  for (index = 0; index < audio_t->demixing_info.demix_entry_count1; index++)
  {
    if (memcmp(demix_group, audio_t->demixing_info.demix_group_entry[index], size_of_demix_group) == 0)
      break;
  }
  if (index == audio_t->demixing_info.demix_entry_count1)
  {
    memcpy(audio_t->demixing_info.demix_group_entry[index], demix_group, size_of_demix_group);
    audio_t->demixing_info.demix_entry_count1++;
  }

  audio_t->demixing_info.size_of_demix_group_entry = size_of_demix_group;
  audio_t->demixing_info.demix_group_entry_pointer = index;
...
int mov_write_audio2(MOVMuxContext *movm, int trak, AVPacket *pkt)
{
  ...
  if (!IS_FRAGMENT_MP4)
  {
    ...
    if (((audio_t->frame.entries + 1) * sizeof(*(audio_t->frame.sizes))) > audio_t->frame.buffersize)
    {
      ...
      audio_t->demixing_info.demix_sample_count = (uint32_t *)_drealloc((char *)audio_t->demixing_info.demix_sample_count, audio_t->frame.buffersize, __FILE__, __LINE__);
      audio_t->demixing_info.demix_group_description_index = (uint32_t *)_drealloc((char *)audio_t->demixing_info.demix_group_description_index, audio_t->frame.buffersize, __FILE__, __LINE__);
    }
    audio_t->frame.offsets[audio_t->frame.entries] = movm->mdatoffset + pos;
    ...
    ////////////////////// demix mode/////////////////////
    mp4_demix_info2(audio_t, pkt->size_of_demix_group, pkt->demix_group);
    //////////////////////////////////////////////////////
  }
  else
  {
    ...
    memcpy(audio_t->frame.fmdat + audio_t->frame.fmdat_offset, pkt->buf, pkt->size);
...
int mov_write_close(MOVMuxContext *movm)
{
  ...
  else
  {
    ...
    if (audio_t->frame.fmdat)
    {
      for (int j = 0; j < 8; j++)
      {
        _dfree(audio_t->demixing_info.demix_group_entry[j], __FILE__, __LINE__);
      }
      _dfree(audio_t->demixing_info.demix_sample_count, __FILE__, __LINE__);
      _dfree(audio_t->demixing_info.demix_group_description_index, __FILE__, __LINE__);

      _dfree(audio_t->ia_descriptor.mix_presentations_group_entry, __FILE__, __LINE__);
      _dfree(audio_t->ia_descriptor.audio_elements_group_entry, __FILE__, __LINE__);
...
int mov_write_trak(MOVMuxContext *movm, int num_video_traks, int num_audio_traks)
{
	...
	{ //
      ...
      if(movm->codec_id == CODEC_OPUS)
        audio_t[i].iamf.csc = _dcalloc(1, sizeof(OpusHeader), __FILE__, __LINE__);
      else if (movm->codec_id == CODEC_AAC)
        audio_t[i].iamf.csc = _dcalloc(1, sizeof(AacHeader), __FILE__, __LINE__);

      for (int j = 0; j < 8; j++)
      {
        audio_t[i].demixing_info.demix_group_entry[j] = (uint8_t *)_dmalloc(256, __FILE__, __LINE__);
      }
      audio_t[i].demixing_info.demix_sample_count = (uint32_t *)_dmalloc(audio_t[i].frame.buffersize, __FILE__, __LINE__);
      audio_t[i].demixing_info.demix_group_description_index = (uint32_t *)_dmalloc(audio_t[i].frame.buffersize, __FILE__, __LINE__);

      audio_t[i].ia_descriptor.mix_presentations_group_entry = (uint8_t *)_dmalloc(1024, __FILE__, __LINE__);
      audio_t[i].ia_descriptor.audio_elements_group_entry = (uint8_t *)_dmalloc(1024, __FILE__, __LINE__);
