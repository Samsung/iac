...
static int mov_read_opus(mp4r_t *mp4r, int size)
{
    ...
    atr[sel_a_trak].csc = _dcalloc(1, sizeof(OpusHeader), __FILE__, __LINE__);
    OpusHeader *header = (OpusHeader *)atr[sel_a_trak].csc;

    header->magic_id = ATOM_TYPE_OPUS;
    header->media_type = mp4r->trak_type[mp4r->cur_r_trak];
...
static int mov_read_aiac(mp4r_t *mp4r, uint32_t size)
{
    ...
    uint32_t sample_rate = avio_rb32() >> 16;
    atr[sel_a_trak].samplerate = sample_rate;
    sscount = avio_r8(); // sub-bitstream count

    atr[sel_a_trak].csc = _dcalloc(1, sizeof(OpusHeader), __FILE__, __LINE__);
    header = (OpusHeader *)atr[sel_a_trak].csc;
    /* header->magic_id = ATOM_TYPE_OPUS; */
    header->media_type = mp4r->trak_type[mp4r->cur_r_trak];
    /* fprintf(stderr, */
            /* "aiacin: index %d, channel count %d, sample size %d, sample rate %u, sub-bitstream count %d\n", */
            /* data_reference_index, channel_count, sample_size, sample_rate, */
            /* sscount); */
...
int mov_read_sgpd (mp4r_t *mp4r, int size)
{
    ...
    if (!memcmp("dmix", buf, 4)) {
        OpusHeader *header = (OpusHeader *)atr[sel_a_trak].csc;
        header->entry_count = count;
        if (header->demix_entries)
        ...
        for (i=0; i<count; ++i) {
            ...
            if (len != 1) {
                fprintf(stderr, "%s description len %d, is not 1.\n",
                        buf, len);
                break;
            }
            header->demix_entries[i] = avio_r8();
            /* fprintf(stderr, "sgpd (dmix): entry %d: %d\n", i, header->demix_entries[i]); */
        }
    } else if (!memcmp("iant", buf, 4)) {
        OpusHeader *header = (OpusHeader *)atr[sel_a_trak].csc;
        len = default_len;
        if (v == 1 && !default_len)
            len = avio_rb32();
        ...
        header->len = len;
        /* fprintf(stderr, "sgpd (iant): meta data size %d\n", len); */

        {
            uint8_t *mt = header->metadata;
            uint8_t flag;
            mt += 19;

            ++mt;
            header->meta.ambisonics_mode = *mt >> 6 & 0x3;
            header->meta.channel_audio  = *mt >> 3 & 0x7;
            ++mt;

            /* fprintf(stderr, "ambisonics_mode %d\nchannel_audio %d\n", */
                    /* header->meta.ambisonics_mode, header->meta.channel_audio); */

            if (header->meta.ambisonics_mode == 2 ||
                    header->meta.ambisonics_mode == 3) {
                return ERR_UNSUPPORTED;
            }

            if (header->meta.channel_audio) {
                header->meta.alc = _dcalloc(header->meta.channel_audio,
                                       sizeof(AudioLayerConfig), __FILE__, __LINE__);
                header->meta.alc_cnt = header->meta.channel_audio;

                AudioLayerConfig *alc;
                for (int i=0; i<header->meta.channel_audio; ++i) {
                    alc = (AudioLayerConfig * )header->meta.alc;
                    alc[i].loudspeaker_layout = mt[0] >> 4 & 0x0F;
                    alc[i].dmix_gain_flag = mt[0] >> 3 & 0x01;
                    alc[i].recon_gain_flag = mt[0] >> 2 & 0x01;

                    alc[i].stream_count = mt[1] ;
                    alc[i].coupled_stream_count = mt[2];
                    alc[i].loudness = mt[3] << 8 | mt[4];

                    mt += 5;
                    if (alc[i].dmix_gain_flag) {
                        alc[i].dmix_gain = mt[0] << 8 | mt[1];
                        mt += 2;
                    }

#if 0
                    fprintf(stderr, "\tloudspeaker_layout %d\n", alc[i].loudspeaker_layout);
                    fprintf(stderr, "\tdmix_gain_flag %d\n", alc[i].dmix_gain_flag);
                    fprintf(stderr, "\trecon_gain_flag %d\n", alc[i].recon_gain_flag);
                    fprintf(stderr, "\tstream_count %d\n", alc[i].stream_count);
                    fprintf(stderr, "\tcoupled_stream_count %d\n", alc[i].coupled_stream_count);
                    fprintf(stderr, "\tloudness %d\n", alc[i].loudness);
                    if (alc[i].dmix_gain_flag) {
                        fprintf(stderr, "\tdmix_gain %d\n", alc[i].dmix_gain);
                    }
...
int mov_read_sbgp (mp4r_t *mp4r, int size)
{
    ...
    if (!memcmp("dmix", buf, 4)) {
        ...
        for (i=0; i<count; ++i) {
            ...
            /* fprintf(stderr, "sbgp (dmix): entry %d, count %d, reference index %d(%d)\n", */
                    /* i, cnt, ridx, header->demix_entries[ridx - 1]); */
            for (int k=0; k<cnt; ++k)
                header->demix_modes[header->dents + scnt + k] = header->demix_entries[ridx - 1];
            scnt += cnt;
        }
